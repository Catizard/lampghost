	...
	<n-grid :cols="12" min-height="600px">
		<n-gi :span="4">
			<n-dropdown trigger="hover" :options="difftableStatusOptions" @select="handleChangeDifftableStatusOption">
				<n-button :loading="difftableChangingLoading">{{
					currentChoosingDifftableName
				}}</n-button>
			</n-dropdown>
		</n-gi>
	</n-grid>
		...
		.then((mainUser) => {
			FindDiffTableHeaderList()
				.then((result) => {
					...
					difftableStatusOptions.value =
						difftableHeaderList.value.map((header) => {
							return {
								label: header.Name,
								key: header.ID,
							};
					...
					QueryUserInfoWithLevelLayeredDiffTableLampStatus(
						...
						notification.error({
							content:
								t("message.loadUserDataErrorPrefix") + err,
							duration: 3000,
...
function handleChangeDifftableStatusOption(key) {
	...
	QueryUserInfoWithLevelLayeredDiffTableLampStatus(mainUser.value.ID, key)
		.then((result) => {
			if (result.Code != 200) {
				return Promise.reject(result.Msg);
			}
